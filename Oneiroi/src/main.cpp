...
struct _OneiroiAlgorithm_DTC
{
    ...
    PatchState patchState;
        ...
        // Debug tracking
        uint32_t stepCount;
        uint32_t looperProcessedCount;
        bool looperLastEnabled;
        bool looperProcessedThisStep;
...
static const _NT_parameter	parameters[] = {
    ...
    // Module Enables
    { .name = "Looper", .min = 0, .max = 1, .def = 1, .unit = kNT_unitEnum, .scaling = 0, .enumStrings = enumStringsOnOff }
...
static const uint8_t page9[] = { kParamLooperEnable };
...
static const _NT_parameterPage pages[] = {
        ...
        { .name = "Modules", .numParams = ARRAY_SIZE(page9), .params = page9 },
...
_NT_algorithm*	construct( const _NT_algorithmMemoryPtrs& ptrs, const _NT_algorithmRequirements& req, const int32_t* specifications )
{
    ...
    alg->dtc->patchState.speedZero = 0.f;
        ...
        alg->dtc->stepCount = 0;
        alg->dtc->looperProcessedCount = 0;
        alg->dtc->looperLastEnabled = true;
        alg->dtc->looperProcessedThisStep = false;
...
}void step( _NT_algorithm* self, float* busFrames, int numFramesBy4 )
{
    ...
    // Debug tracking
    dtc->stepCount++;
    dtc->looperLastEnabled = (pThis->v[kParamLooperEnable] > 0);
    dtc->looperProcessedThisStep = false;
    
    // Process audio (conditionally for looper)
    if (dtc->looperLastEnabled) {
        dtc->Oneiroi_->Process(*myBuffer);
        dtc->looperProcessedThisStep = true;
        dtc->looperProcessedCount++;
    } else {
        // Process without looper - need to call individual modules
        // For now, just skip everything to test the enable/disable
        myBuffer->clear();
    }
